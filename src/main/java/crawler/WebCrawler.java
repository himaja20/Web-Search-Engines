...
public class WebCrawler {
	...
	public boolean robotSafe(URL url) {
		...
		int index = 0;
		int agentIndex = 0;
		while ((agentIndex = strCommands.indexOf(AGENT, index)) != -1) {
			agentIndex += AGENT.length();
			index = strCommands.indexOf(DISALLOW, agentIndex);
	...
	while ((index = lcPage.indexOf("<a",index)) != -1) {
		//TODO have to start from checking correct locations of <a href tags
		ihref = lcPage.indexOf("href",index);
		iEndAngle = lcPage.indexOf(">",ihref);
		if (index != -1) {
